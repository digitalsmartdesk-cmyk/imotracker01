...
function RadarChart() {
  ...
  const outerPoints = OUTER_POINTS.map(p => `${p.x},${p.y}`).join(' ')
  return (
          ...
          <div style={{ padding: '0 32px 24px' }}>
            ...
            <div className="rg-3up-sm">
              {[
                ...
                <div key={i.title} style={{ background: i.color, border: `1px solid ${i.border}`, borderRadius: 14, padding: '18px 20px' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--teal)', marginBottom: 8 }}>{i.label}</div>
                  <h4 style={{ fontSize: 14, fontWeight: 700, color: 'var(--navy)', marginBottom: 8 }}>{i.title}</h4>
                  <p style={{ fontSize: 13, color: 'var(--navy-ink)', opacity: 0.7, lineHeight: 1.55 }}>{i.body}</p>
                </div>
              ))}
            </div>
          </div>
