      ...
      {[1, 2, 3, 4].map(n => (
        <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            ...
            background: n < step ? 'var(--teal)' : n === step ? 'var(--navy)' : 'var(--line)',
            color: n <= step ? 'white' : 'var(--navy)',
            border: n === step ? '2px solid var(--navy)' : 'none',
          }}>
            {n < step ? '✓' : n}
          </div>
          {n < 4 && <div style={{ width: 32, height: 2, background: n < step ? 'var(--teal)' : 'var(--line)', borderRadius: 1 }} />}
        </div>
      ))}
    </div>
  )
      ...
      <div style={{ width: '100%', maxWidth: 520 }}>
        ...
        {step === 4 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 800, color: 'var(--navy)', margin: '20px 0 12px' }}>You're all set!</h1>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <button className="btn btn-coral btn-lg" onClick={finish}>Take {selectedTest === 'emotional-awareness' ? 'the free check-in' : 'first check-in'} →</button>
